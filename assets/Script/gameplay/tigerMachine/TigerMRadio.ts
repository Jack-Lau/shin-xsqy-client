...
@ccclass
export default class TigerMRadio extends cc.Component {
    ...
    async init() {
        this.text.string = '点击摇一次，马上领奖！';

        let config = (await ConfigUtils.getConfigJson('BroadcastInfo'));
        this.wabaoBuriedConfig = R.prop(3290003, config);
    ...
    async toBuried() {
        ...
        let response = await NetUtils.sendHttpRequest(NetUtils.RequestType.GET, '/chat/latestInterestingMessage/3290003', []) as any;
    ...
    async updateLabel() {
        ...
        let groupName = ' <color=#ffb94c>' + R.path(['args', 'groupName'], data1) + '</color> ';
        let data2 = this.wabaoBuriedConfig.description.toString().replace('${playerName}', '<color=#fff2aa> ' + playerName2 + ' </color>');
        let data3 = data2.toString().replace('${groupName}', groupName);
        this.text.string = data3;
