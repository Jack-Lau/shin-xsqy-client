...
@ccclass
export default class ChangeSchoolPanel extends cc.Component {
    @property(Item)
    skill1: Item = null;
    @property(Item)
    skill2: Item = null;
    ...
    start() {
        ...
        this.confirmBtn.node.on(cc.Node.EventType.TOUCH_END, CommonUtils.aloneFunction(this.changeSchool.bind(this)));
	...
	getIndex(schoolId: number) {
        if (schoolId === 104) {
            return 1;
        } else if (schoolId === 102) {
            return 2;
        } else if (schoolId === 101) {
            return 3;
        } else {
            return 4;
        }
    ...
    async changeSchool() {
		...
		if (PlayerData.getInstance().schoolId === schoolId) {
			TipsManager.showMessage('转换的目标门派就是少侠您的当前门派');
			return;
		}
		//
		let callback = async () => {
			let response = await NetUtils.sendHttpRequest(NetUtils.RequestType.POST, '/school/redeemChangeSchool', [20086, schoolId]) as any;
			if (response.status == 0) {
				TipsManager.showMessage('转换门派成功！');
		...
		//
		let schoolName = this.resources[this.currentIndex]['schoolName'];
		CommonUtils.showRichSCBox(
            `是否花费一本<img src='currency_icon_163'/>拜入 <color=#187122>${schoolName}</c> 门下？`,
            `(门派技能、天赋、修炼将完全保留)`,
            "少侠的当前门派为 " + this.resources[this.getIndex(PlayerData.getInstance().schoolId)]['schoolName'],
            callback
        );
    }
